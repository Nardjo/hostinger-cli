...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  hostinger-cli auth show\n  hostinger-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: hostinger-cli auth set <token>");
      return;
    }
...
authCommand
  ...
  .action(async () => {
    try {
      await client.get("/api/vps/v1/virtual-machines");
